...
function makeQuery(data: unknown) {
  ...
  for (const m of ["select", "eq", "gte", "order", "limit", "in", "or", "not"]) {
    q[m] = () => q;
  }
  q.maybeSingle = () =>
    Promise.resolve(
      Array.isArray(data) ? { data: data[0] ?? null, error: null } : { data, error: null }
    );
  q.then = ((resolve: (v: unknown) => unknown) =>
    Promise.resolve({ data, error: null }).then(resolve)) as (...args: unknown[]) => unknown;
...
function createMockSupabase(opts: { dispatchLogRows?: unknown[] } = {}) {
  ...
  return {
    from: vi.fn((table: string) => makeQuery(tableData[table] ?? null)),
    auth: {
      admin: {
...
describe("GET /api/cron/alert-dispatch (recipient honesty)", () => {
  ...
  it("hasRecentDispatch uses .eq() for text references (no PostgREST parse error)", async () => {
    ...
    const loggedFrom = createMockSupabase({ dispatchLogRows: [{ id: "logged" }] });
    await expect(
