...
![DevPulse dashboard](design/dashboard.png)

DevPulse is a full-stack Next.js application that turns continuously collected job-market data into a practical developer intelligence workflow: measure demand, compare your skills against real postings, identify gaps, and move directly into targeted learning.
...
- **Live market data:** Daily ingestion from 10 job sources with monthly skill-demand snapshots.
- **Deterministic matching:** Job stack-match and market-alignment scores are calculated from stored data, not generated by AI.
- **Grounded AI:** AI summarizes and explains computed results instead of inventing market statistics.
- **Learning loop:** Skill gaps lead to ranked YouTube tutorials with chapter-first timestamp matching.
- **Persistent profiles:** Supabase Auth stores a developer's target role, skills, preferences, saved jobs, and optional GitHub-synced skills.
- **Server-side integrations:** API keys stay on the server; ingestion, AI calls, GitHub sync, tutorial indexing, and alerts run through protected server routes and cron jobs.
...
The repository includes the original product reference screens used to drive implementation:

- [Dashboard](design/dashboard.png)
- [Trends](design/trends.png)
- [Jobs](design/job-description.png)
- [Skills](design/skills.png)
- [Gap Report](design/gap-report.png)
- [Profile](design/user-profile.png)
...
```bash
git clone https://github.com/prg-04/dev-pulse.git
cd dev-pulse
pnpm install
